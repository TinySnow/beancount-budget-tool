...
**核心能力:** YAML 嵌套预算、模板继承（`default_monthly`）、多套模板切换、父子桶自动聚合、四种统计范围（月/累计/区间/年）

**自动归类:** `mappings.yml` 前缀匹配 → 自动归桶，未标注 `budget:` 的消费默认归入生活费桶，无需每笔手写标签

**手工精细控制:** 多桶归属（`budget: "数码, 爱好"`）、桶名:金额拆分、Income 入账抵消、简写桶名自动补全、跟踪桶（`planned=0`）

**资产追踪:** 资产桶资金位置、储蓄/投资进出流追踪、超支隐藏持仓段

**输出:** 中文化报告、Markdown/CSV/TXT/JSON 导出、横向月度透视 CSV、同比对比
